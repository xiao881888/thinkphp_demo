...
class UserCouponModel extends Model{
	...
	public function getCouponBalance($userCouponId){
		...
		return $this->where($condition)->getField('user_coupon_balance');
	}
	...
	public function deductCoupon($uid, $userCouponId, $money){
		...
		$condition['user_coupon_balance'] = array(
				'egt',
				$deduct 
		);
        $remark = '';
		...
		if ($deduct > 0) {
			$result = $this->where($condition)->setDec('user_coupon_balance', $deduct);
			$consume = D('UserAccount')->updateConsumptionCouponStatics($uid, $deduct);
			$log_result = D('UserCouponLog')->addUserCouponLog($uid, $userCouponId, $deduct, $couponBalance - $deduct, C('USER_COUPON_LOG_TYPE.USE'), $uid, $remark);
		} else {
			return 0;
		}
	...
	public function increaseCoupon($userCouponId, $money){
		...
		$result = $this->where($condition)->setInc('user_coupon_balance', $money);
	...
	public function deductCouponBalance($uid, $user_coupon_id, $money, $log_type, $remark=''){
		$user_coupon_balance = $this->getCouponBalance($user_coupon_id);
		$map['uid'] = $uid;
		$map['user_coupon_id'] = $user_coupon_id;
		$result = $this->where($map)->setDec('user_coupon_balance', $money);
		$consume = D('UserAccount')->updateConsumptionCouponStatics($uid, $money);
		$log_result = D('UserCouponLog')->addUserCouponLog($uid, $user_coupon_id, $money, $user_coupon_balance - $money, $log_type, $uid, $remark);
		if ($result && $consume && $log_result) {
			return $money;
		} else {
			return false;
	...
	public function getUserCouponInfoByAndUidIssueId($uid, $coupon_id, $issue_id){
		$where['uid'] = $uid;
	...
	public function getUserCouponCountById($uid, $coupon_id){
		$where['uid'] = $uid;
		$where['coupon_id'] = $coupon_id;
