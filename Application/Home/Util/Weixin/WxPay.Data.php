...
class WxPayDataBase
{
	...
	**/
	...
	{	
		...
		ApiLog('from xml:'.print_r($xml,true),'wx');
        ...
        ApiLog('from xml $this->values:'.print_r($this->values,true),'wx');
	...
	public static function buildSignForParams($params)
	{
		//签名步骤一：按字典序排序参数
		ksort($params);
		$string = $this->buildUrlParams($params);
		//签名步骤二：在string后加入KEY
		$string = $string . "&key=".WxPayConfig::KEY;
		//签名步骤三：MD5加密
		$string = md5($string);
	...
	public static function buildUrlParams($params)
	{
		$buff = "";
...
class WxPayResults extends WxPayDataBase
{
	...
	public function CheckSign()
	{
		...
		$sign = $this->MakeSign();
		ApiLog('post :'.$this->GetSign().'==='.$sign, 'wx');
	...
	{	
		...
		ApiLog('ret code:'.$obj->values['return_code'],'wx');
		$obj->CheckSign();
        return $obj->GetValues();
	}
}
