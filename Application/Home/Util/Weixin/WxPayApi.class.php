...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 30)
	{
		...
		ApiLog('post $$response:'.print_r($response,true), 'wx');
		ApiLog('post $result:'.print_r($result,true), 'wx');
...
// 		curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,2);//严格校验
		...
		ApiLog('curl request:'.$url.'==='.print_r($xml,true), 'wx');
		...
		if($data){
			curl_close($ch);
			ApiLog('curl data:'.print_r($data,true), 'wx');
				
			return $data;
		} else { 
			$error = curl_errno($ch);
			curl_close($ch);
			ApiLog('curl error:'.$error.'===='.curl_error($ch), 'wx');
