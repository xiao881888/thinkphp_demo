...
class LLpayNotify {
	 ...
	 * @return 验证结果
	 */
	function verifyReturn($post) {
		if (empty ($post)) { //判断POST来的数组是否为空
			return false;
		} else {
			//首先对获得的商户号进行比对
			if (trim($post['oid_partner']) != $this->llpay_config['oid_partner']) {
				//商户号错误
				return false;
			}
			...
			//生成签名结果
			$parameter = array (
				'oid_partner' 	=> $post['oid_partner'],
				'sign_type' 	=> $post['sign_type'],
				'dt_order' 		=> $post['dt_order'],
				'no_order' 		=> $post['no_order'],
				'oid_paybill' 	=> $post['oid_paybill'],
				'money_order' 	=> $post['money_order'],
				'result_pay' 	=> $post['result_pay'],
				'settle_date' 	=> $post['settle_date'],
				'info_order' 	=> $post['info_order'],
				'pay_type'		=> $post['pay_type'],
				'bank_code'		=> $post['bank_code'],
			);
			if (!$this->getSignVeryfy($parameter, trim($post['sign']))) {
				return false;
			}
			return true;
